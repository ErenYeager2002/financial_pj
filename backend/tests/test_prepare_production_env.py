...
def test_monorepo_paths_are_the_portable_defaults() -> None:
    assert production_env.default_next_root(PROJECT_ROOT) == PROJECT_ROOT / "web"
    compose = (PROJECT_ROOT / "deploy" / "production" / "compose.yaml").read_text(
        encoding="utf-8"
    )
