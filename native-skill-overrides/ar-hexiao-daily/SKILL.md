...
本安装版本为 2026.09.16.1-platform.1，业务脚本与独立包一致。平台中的执行方式优先按本节：

- Skill 目录是 `/skill`，只读；命令使用 `cd /skill` 或脚本绝对路径。
- 输入来自当前会话上传清单的 `/workspace/inputs/` 路径。根据上传清单的原始文件名识别年度及用途，复制时恢复明确年度文件名；不要根据随机文件ID推断年份。
- 业务工作区固定放在 `/workspace/outputs/ar-work`。只有 outputs 下的文件会在命令结束后保存，后续命令才能继续使用。先创建子目录，再把选定输入复制到该工作区；原始上传文件保持只读。发布结果包含工作副本和核验记录。
- 当前沙箱无网络、无智云凭据。跳过下面的联网取数命令，要求提供本次日期的完整智云导出；缺少导出时说明缺少哪些材料，不尝试联网、不索要明文密码。不要安装网络依赖或修改沙箱权限。
- 每条命令最多运行120秒，工作空间512MiB；按步骤拆成多条命令。超过容量或单步骤时限的批次报告限制，不能宣称任务完成，也不要自动重复失败的财务写入。
- 用户明确要求核销后，仍先生成日清、通过写前校验，再写隔离工作副本并回读。创建、替换、检查Skill不构成真实核销授权。
