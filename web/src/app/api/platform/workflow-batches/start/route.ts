...
function parseBody(value: unknown): {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new PlatformApiError(400, '批量后台任务请求格式无效。');
  }
  ...
  return { skill_id: skillId, reconciliation_dates: dates, files: parseFiles(body.files) };
}
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    try {
      ...
      throw new PlatformApiError(400, '批量后台任务请求不是有效的 JSON。');
    }
    ...
    return platformRouteError(error, '批量后台任务启动失败。');
  }
}
