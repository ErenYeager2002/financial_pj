...
function fixture() {
  ...
  const dependencies: Record<string, unknown> = {
    ...
    '@/features/ai-chat/api/server': { appendAssistantMessage: async (_s: string, role: string, content: string) => {
      if (role === 'assistant') { messages.push(content); saved.resolve(); }
    } },
  ...
  return { exports, gate, saved, messages, isAborted: () => aborted, setOwner: (value: string) => { owner = value; } };
}
