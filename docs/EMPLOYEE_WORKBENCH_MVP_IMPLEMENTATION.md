...
基线方案中的安全阻断项包括：`frontend/src/api.ts` 根据 URL 生成管理员身份；`backend/app/auth.py` 信任身份请求头；
文件下载、任务和工作流读取主要按部门放行。以下状态记录用于区分该基线与当前实现。

### 当前状态记录（2026-08-16）

- `ar-hexiao-daily` 已同步为 1.5.1 并登记为 `published`，使用 `workflow` 适配器。
- 该 Skill 保留确认、变更复核和管理员审批三道门槛；真实智云取数仍依赖部署环境配置，当前自动化回归使用合成数据。
- 生产库中的附件记录、上传目录和历史 Skill 工作区均已从活动平台清理；为便于恢复，清理前的物理文件已移到项目外隔离留存目录，不参与运行时读取。
